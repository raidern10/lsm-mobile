<x-app-layout>
    <div class="max-w-3xl mx-auto py-6 px-4">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-xl font-semibold text-gray-800">Edit Lembar Observasi</h1>
        ...
        <form action="{{ route('guru.observasi.update', $observasi->id) }}" method="POST"
            ...
            <div>
                ...
                <select name="user_id" required
                    ...
                    @foreach ($siswas as $s)
                        <option value="{{ $s->id }}" @selected(old('user_id', $observasi->user_id) == $s->id)>
                            {{ $s->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" name="hari_tanggal"
                       value="{{ old('hari_tanggal', $observasi->hari_tanggal ? \Carbon\Carbon::parse($observasi->hari_tanggal)->format('Y-m-d') : '') }}" required
                       class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Pekerjaan / Projek</label>
                <input type="text" name="pekerjaan_projek"
                       value="{{ old('pekerjaan_projek', $observasi->pekerjaan_projek) }}"
                       class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500"
